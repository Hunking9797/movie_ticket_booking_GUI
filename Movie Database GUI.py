...
def book():
    try:
        ...
        if(customer_name_entry.get() == "" or customer_id_entry.get() == "" or customer_age_entry.get() == "" or movie_select_entry.get() == ""):
            ...
            con = mysql.connect(host='localhost', user='root', password='arunnaveen', database='movie')
            cursor = con.cursor()
            ...
            for row in rows: 
                ...
                insertData = f"{str(row[0]):>15}{str(row[1]):>24}{str(row[2]):>25}{str(row[3]):>30}"
                table_content.insert(table_content.size()+1, insertData)

            messagebox.showinfo(title='Info', message='Booked Successfully!')
            con.close()
    except ValueError as ve:
        error = Label(window,text='Invalid input',font=('System',15), fg="white", bg='grey')
        error.place(x=200,y=400)
        print('Values should be appropriate',ve)
        error.grid_remove()
